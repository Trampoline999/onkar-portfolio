...
export const BentoCard = ({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col overflow-hidden rounded-2xl md:rounded-2xl text-card-foreground transition-all duration-300 hover:-translate-y-1 hover:shadow-xl group",
        className,
      )}
    >
      ...
      <div className={cn("relative flex min-h-0 flex-1 flex-col gap-3 p-6 sm:p-7", innerClassName)}>
        ...
        <div className="space-y-3 relative z-10">
          {title ? (
            <h3
              className={cn(
                "font-bricolage text-xl font-semibold leading-tight sm:text-xl md:text-2xl lg:text-3xl text-zinc-800 dark:text-zinc-800",
                titleClassName,
              )}
            >
              {title}
